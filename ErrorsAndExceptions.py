...
# в последней строчке выводится имя исключение
...
# она не вы водит строки прочитанные из стандартного ввода
...
# если во время выполнения блока try
# возникает исключение и имя исключения совпадает
# с хотя бы одним ключевым словом, указанным после слова except

# выполение оставшейся части блока try пропускается
# и выполняется блок except, в котором найдено совпадение
...
# класс каждого исключения является производным классом другого исключния
# но не наоборот
...
# такие как SystemExit (вызваеься функцией sys.exit()) и KeyboardInterrupt
...
# так как except A сработвет для всех данных случаев

# ----------------------------------------------------------------------------

# ключевое слово raise

# завершает ход выполнения программы, вызывает указанное исключение
# в блоке try переходит к except

# ----------------------------------------------------------------------------

# при возникновении исключений
# могут возникать связанные с ними значения, так называемые
# аргумента исключений

# наличие и типы зависят от самого аргумента

# при указании исключений после имени исключения можно указать переменные
...
# почти всех/всех исключений

# но рекомендуется как можно точнее указывать исключение,
# что бы избежать неожиданных исключений
...
# excs - последовательность (может быть любая, желательно кортеж) исключений
...
#     nothing()
#     ~~~~~~~^^
#   File "c:\Projects\LearningPython\ErrorsAndExceptions.py", line 165, in nothing
#     raise BaseException('abc', 123)
